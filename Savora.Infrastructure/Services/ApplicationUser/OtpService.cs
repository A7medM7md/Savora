    ...
    public static class OtpService
    {
        ...
        public static bool ValidateCode(SecurityToken securityToken, int code, string? modifier = null, int numberOfDigits = 6)
        {
            ...
            using (var hashAlgorithm = new HMACSHA1(securityToken.GetDataNoClone()))
            {
                for (var i = -2; i <= 2; i++)
                {
                    if (ComputeTotp(hashAlgorithm, timestepNumber + (uint)i, modifier, numberOfDigits) == code)
                    {
                        return true;
                    }
                }
        ...
        private static uint GetCurrentTimeStepNumber()
        {
            var delta = DateTime.UtcNow - UnixEpoch;
            return (uint)(delta.Ticks / Timestep.Ticks);
        }

        private static int ComputeTotp(HashAlgorithm hashAlgorithm, uint timestepNumber, string? modifier, int numberOfDigits)
        {
            var mod = (int)Math.Pow(10, numberOfDigits);
            var timestepAsBytes = BitConverter.GetBytes(IPAddress.HostToNetworkOrder((int)timestepNumber));
